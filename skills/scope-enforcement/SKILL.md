...
description: "Prevents AI agent scope creep by enforcing driftlock boundaries before, during, and after every coding task. Supports file-level and function-level locking. Use before writing any code, before every commit, and before any review or ship phase. Requires driftlock CLI (npm install -g driftlock)."
---
...
This skill uses `driftlock` — a zero-dependency Node.js CLI — to enforce two tiers of scope protection:

- **File-level:** An entire file is locked. No agent may touch it.
- **Function-level:** A specific function within a file is locked. The file may be edited, but that function's body is off-limits. `driftlock guard` detects if any changed line falls inside the locked function's boundaries.
...
**Step 1: Verify driftlock is initialized.**
```bash
driftlock status
```
...
```bash
driftlock context "<task description>"
```
...
driftlock guard   # must pass before committing
```
...
Open `.driftlock.json` and review every `history` entry. Every unlock must have a specific, task-justified reason. Vague entries like "needed it" indicate unreviewed scope creep.

**Step 7: Re-lock completed code.**
```bash
driftlock lock src/auth/token.ts:validateToken "fixed and re-locked"
```
...
```bash
echo '#!/bin/sh
driftlock guard' > .git/hooks/pre-commit
...
| Excuse | Rebuttal |
|--------|----------|
| "The locked function only had a minor change." | Size is irrelevant. Run `git restore` or use `driftlock unlock <file>:<func>` with a real reason. |
| "I'll check scope after I finish the implementation." | Checking after the fact means the violation already happened. Start with `driftlock context`. |
...
| "Running `driftlock guard` slows my commit flow." | Under 300ms. The alternative is debugging a hallucinated refactor for hours. |
| ".driftlock.json shouldn't be committed." | Commit it. It's shared project state — your whole team and their agents benefit. |

---

## Red Flags

- `driftlock guard` reports violations in more than 2 files — significant scope drift.
...
- `driftlock status` shows 0 locked files when you expected protection — manifest may be stale or deleted.
...
- [ ] `.driftlock.json` exists and `driftlock status` shows the expected locks.
- [ ] `driftlock context "<task>"` output was injected into the agent before any code was written.
- [ ] `driftlock guard` exits `0` before every commit in this session.

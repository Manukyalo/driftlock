...
Before you let an agent loose on a codebase, you need a guaranteed escape hatch. `driftlock save` creates that escape hatch using git's native stash mechanism. It is instant, requires no external tools, and can restore the repo to its exact pre-session state in under one second.

## How Storage Works

- The snapshot is stored in **git's local stash** (`.git/refs/stash`) — it never leaves your machine and is never pushed to GitHub.
- The `.driftlock.json` manifest stores a pointer (`lastSnapshot: "dirty" | "clean"`) so `driftlock restore` knows what to restore.
